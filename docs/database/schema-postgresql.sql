...
COMMENT ON TABLE negocio IS 'Representa organizaciones o empresas que contienen sistemas';
COMMENT ON TABLE sistema IS 'Sistemas o aplicaciones dentro de un negocio';
COMMENT ON TABLE menu IS 'Menús dentro de un sistema, soporta jerarquía';
COMMENT ON TABLE opcion IS 'Opciones de menú (enlaces, acciones), soporta jerarquía';
COMMENT ON TABLE usuario IS 'Usuarios del sistema';
COMMENT ON TABLE rol IS 'Roles o perfiles de usuario';
COMMENT ON TABLE usuario_rol IS 'Relación muchos a muchos entre usuarios y roles';
COMMENT ON TABLE visibilidad IS 'Reglas de visibilidad extensibles para opciones según roles';
COMMENT ON TABLE acceso IS 'Historial de accesos para auditoría';
